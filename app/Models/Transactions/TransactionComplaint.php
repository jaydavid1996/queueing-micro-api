...
class TransactionComplaint extends Transaction
{
    ...
    protected $hidden = ['payment_status', 'payment_date'];

    protected static function boot()
    {
        parent::boot();
        static::addGlobalScope('type', function (Builder $builder) {
            $builder->where('type',  TransactionComplaint::TYPE);
        });


        static::creating(function ($model) {
            if (empty($model->type)) {
                $model->type = TransactionComplaint::TYPE;
            }
            if(  (auth()->user() !== null) || $model->barangay_id ){
                ...
                $model->reference_id =  IdGenerator::generate([
                    ...
                    'prefix' => $model->barangay_id . '-C' . date('y') . '-'
                ]);
